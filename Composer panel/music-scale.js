...
function selectscale() {
    ...
    if (scale == 'random'){
        ...
        console.log("Random chosen scale : ", scale);
    }
...
function notesOfThisScale(selected_scale){
    ...
    if (majmin == "major"){
        ...
        switch (selected_scale){
            case 'C': 
                baseline = [0,2,4,5,7,9,11]; // C - no DIESIS
                rootnote = 0; 
                break;
                case 'C#':
                baseline = [0,1,3,5,6,8,10]; // C# - DIESIS: do,re,mi,fa,sol,la,si
                rootnote = 1;
                break;            
                case 'D':
        ...
        console.log(numbers_of_notes.length);
